...
import { FileText, BookOpen, Download, Search, Filter, X } from "lucide-react";
...
const Articles = () => {
  ...
  const categories = useMemo(() => {
    const set = new Set(merged.map((a) => a.category));
  ...
  const filtered = useMemo(() => {
    let result = merged;
    if (selectedYear !== "all") {
      result = result.filter((a) => a.hebrew_year === selectedYear);
    }
    if (selectedCategory) {
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center">
            <h1 className="text-3xl md:text-4xl font-bold mb-2">
              ספריית <span className="text-gold">המאמרים</span>
            </h1>
            <p className="text-muted-foreground text-sm">מאמרי הגות ופרשת שבוע לקריאה והורדה</p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="rounded-xl h-32 animate-pulse bg-muted/20" />
              ))}
            </div>
          ) : filtered.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {filtered.map((article) => (
                ...
                >
                  ...
                  <div className="relative p-5">
                    ...
                    <h3 className="text-foreground font-bold text-lg leading-tight line-clamp-2 group-hover:text-gold transition-colors">
                      {article.title}
                    </h3>
            ...
            <div className="text-center py-16">
              ...
              <h2 className="text-xl font-bold text-foreground mb-2">לא נמצאו מאמרים</h2>
                ...
                {selectedArticle.links.map((url, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-foreground font-semibold text-sm group-hover:text-gold transition-colors leading-tight">
                        {hebrewPart}
                      </span>
